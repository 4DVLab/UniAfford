...
# TODO:remove pointnet2 version, discard.
class PointCloudHiddenStateDecoder(nn.Module):
    """使用文本隐藏状态查询点云特征的解码器。"""
    ...
    def __init__(
        ...
        super().__init__()
        self.config = config
        self.point_encoder = PointCloudEncoder(out_dim=text_hidden_size, config=config)
        ...
        self.point_cloud_segmentor.set_point_encoder(self.point_encoder)
        self.point_cloud_segmentor.set_point_feature_dim(text_hidden_size)
        for param in self.point_cloud_segmentor.parameters():
            param.requires_grad = True
    ...
    def forward(
        ...
        Returns:
            pred_masks: [B, N]，可直接与二值点级 GT 计算 BCEWithLogitsLoss。
        """
        if point_clouds.shape[1] != 3:
            point_clouds = point_clouds.permute(0, 2, 1)
        ...
        # [B, C] -> [B, 1, C]，直接使用上游 FSC 后 text_feat
        text_feat = pred_embeddings.unsqueeze(1)
        text_mask = torch.ones(
            text_feat.shape[0], 1,
            dtype=torch.bool, device=text_feat.device,
