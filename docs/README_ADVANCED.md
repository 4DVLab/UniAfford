...
- `llm_lr`：LLM / LoRA / MLLM 相关参数。默认由 `lr * 0.01` 推导 (1e-5)，保守微调语言语义。
- `vision_2d_lr`：2D decoder 任务头相关参数，默认 `5e-6`。
- `vision_3d_lr`：3D decoder 任务头相关参数，默认 `5e-4`。
- `lr`：router、投影层、非上述分组的其他新参数，默认 `1e-3`。


调整策略遵循 “**能力强的学习率更低**” 原则，能力强指模型经过**预训练**or对下游任务异化的特征有**很强的学习能力**（例如SAM可以依靠图像本身进行分割而不依赖于语义提示），：

- 2D 或 3D 分支明显欠拟合（训练集收敛慢、上限低）：通常是没发挥好下游解码器的性能，优先提高对应任务头学习率；
- 训练 early stage 抖动大：降低 `lr` 或增加 `warmup_num_steps`。

**下游分支能力过强or不依赖语义提示**（Routed Token）：这种情况在我们使用SAM模型进行2D分割的时候遇到过，解决方法是通过图像逐点（dense）特征和语义提示token做相似度的方式，来强制将语义特征和图片特征对齐。相似度锚定的方式能够让最后的结果非常依赖于语义提示token，同时通过降低下游分支学习率的方式让与提示token稳定后再让图像特征对齐，避免由于SAM本身能力很强，导致下游分支在语义token还未收敛的时候走的捷径，直接根据图片本身的信息来对其Affordance做判断（通常表现为训练集上指标很高，但验证集（分布内）和训练集有一段明显的gap，指标明显低于训练集）。

联合学习时，实际上学习率衰减是稀疏分配的
